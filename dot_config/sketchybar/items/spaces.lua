...
local app_icon_overrides = {
	["BrowserOS"] = icons.apps.browseros,
}

local function app_icon_for(app)
	local lookup = app_icon_overrides[app] or app_icons[app]
	return lookup or app_icons["default"]
end
...
space_window_observer:subscribe("space_windows_change", function(env)
	...
	for app, count in pairs(env.INFO.apps) do
		no_app = false
		icon_line = icon_line .. utf8.char(0x202F) .. app_icon_for(app)
	end
...
front_app_icon:subscribe("front_app_switched", function(env)
	front_app_icon:set({ label = { string = app_icon_for(env.INFO), color = colors.accent1 } })
end)
